...
> ⚠️ Attenion !
> Si un container est appelé à utiliser un port déjà pris par une autre application, ce port ne va pas s'exécuter. Exemple, si vous tentez de monter le container spark-master alos que son port 8081 est occupé, vous devez lancer cette commande pour savoir qui utilise ce port:
> ```cmd
> sudo lsof -i :8081
> ```

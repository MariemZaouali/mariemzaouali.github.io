...
Nous allons exécuter l'application **Wordcount** avec Spark.Créez un projet **Wordcount** sur votre IDE.
...
```
Lancez du menu de maven la commande clean package ou à l'aide du terminal, positionnez vous à l'emplacement du `pom.xml` et lancez la commande:
```cmd
mvn clean package
```
...
>📓 NOTEZ BIEN
>Nous ne ferons pas de partitionnement sous Hive. Au départ, l'idée était d'ajouter une nouvelle colonne à chaque table, de l'alimenter avec une date récente et de partitionner les données en fonction de cette date.

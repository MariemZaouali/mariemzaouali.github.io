...
Il faut maintenant activer le PPD sur hive:
```sql
...
| Paramètre                          | Rôle                                                             |
| ---------------------------------- | ---------------------------------------------------------------- |
| **hive.optimize.ppd**              | Active le PPD général de Hive                                    |
| **hive.optimize.ppd.storage**      | Active PPD au niveau du stockage (ORC)                           |
| **orc.splits.include.file.footer** | Permet au moteur de lire le footer ORC, qui contient min/max     |
| **index.filter**                   | Autorise l’utilisation des statistiques pour ignorer des stripes |
...
Stage-1
  Map Operator Tree:
      TableScan
        alias: customers_orc
        PushedPredicate: country = 'France'
        Select Operator
```
`TableScan` : Hive doit scanner la table, mais ce scan sera optimisé grâce au PPD.
 PushedPredicate: country = 'France'
C’est l’élément le plus important. Cela signifie :

- Hive a transmis le filtre au reader ORC
- Le fichier ORC contient des statistiques min/max, bloom filters(ignorer les stripes ne contenant pas country=France), row groups

ORC va éviter de lire tous les blocs dont country ne contient aucune valeur "France".  C’est précisément PPD.
...
```
## A vous de jouer!
Lien drive des fichiers à manipuler:
```sql
https://drive.google.com/drive/folders/1yHi5gPFu_4999GhdemwRD8ux2mllufU8?usp=drive_link
````

...
![image](https://github.com/user-attachments/assets/b9267213-386a-4b01-99e4-598492efce7b)
...
```cmd
 jps
```
![image](https://github.com/user-attachments/assets/0c1b7e23-b5d0-449c-a830-940ca05a742a)
...
Maintenant, nous allons mettre le fichier jar généré par votre application WordCount sur HDFS. Rendez-vous à l'emplacement de ce jar qui ce trouve dans le répertoire target de votre projet. Une fois bien positionné sur le répertoire en question, vous lancez la commande suivante:

```cmd
hadoop fs -put Wordcount2-0.jar
```
Vous lancez par la suite la commande suivante pour exécuter le job sur Hadoop:
...
Il est demandé maintenant de passer d'une architecture namenode + datanode, on veut maintenant créer un autre datanode de manière à avoir namenode + 2 datanodes . Vous devez donc passer par la configuration du fichier `hdfs-site.xml` ainsi que la création des espaces pour chaque datanode. 

## Manipulation 4
On va travailler sur d’autres données. Rendez-vous au site http://archive.ics.uci.edu et chercher la dataset « census income ». 

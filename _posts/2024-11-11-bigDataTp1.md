...
En accédant à n'importe quel conteneur (container) vous êtes capables de lancer les commandes de **hdfs** et avoir le même résultat. Mais avant de lancer la commande sur **hdfs**, nous allons charger le fichier `purchases.txt` sur un des containers puis nous allons le mettre sur **hdfs**. Donc, la première étape consiste en se plaçant au répertoire où se trouve le fichier `purchases.txt`:
```cmd
sudo docker cp purchases.txt namenode:/purchases.txt
```
Maintenant, nous chargeons cee fichier qui se trouve maintenant dans le container **namenode** sur **hdfs**:
```cmd
...
```
Ensuite, nous chargeons le jar `Wordcount2-0.jar` sur le même container en se plaçant au dossier **target** qui se trouve en local et là où il y a le jar généré par votre application et ce en tapant:
```cmd

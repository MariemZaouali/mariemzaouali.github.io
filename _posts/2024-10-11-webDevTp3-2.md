...
```
Maintenant, nous allons faire un peu le ménage! puisque la liste des produits (pièces de robots peuvent être consultées à partir de l'API). C'est pour cette raison, nous allons supprimer la liste des produits codées en dur "hard coded" dans le composant **catalog**.

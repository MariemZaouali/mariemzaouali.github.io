...
Ce scénario est assurée par le panier **Cart**. Dans le TP précédent, nous avons ajouté **Cart** en tant que service. Vous allez supprimer ce service et le remplacer par tout un répertoire appelé **Cart**. Tous comme les autre composants home, site-header etc. Téléchargez-le à partir de lien suivant:
```link

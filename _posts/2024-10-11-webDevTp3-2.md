...
````
Comme nous avons vu dans le TP du NodeJS/Express, la requête GET est utilisée lorsque nous voulons lister les données d'une ressource. Quant à la requête POST, nous l'utilisons lorsque nous allons insérer une nouvelle donnée. Pour mettre en oeuvre ce scénario, nous allons modifier l'implémentation du panier **cart**. Dans le fichier `cart.component.ts`, le constructeur doit ressembler à cela:
```ts

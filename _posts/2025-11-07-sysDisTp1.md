...
Il s'agit d'une architecture réseau comprenant différents composants à savoir pairs, RMI Registry,
annuaire P2P. Pour qu'un pair puisse télécharger un fichier sur d'autres pairs du réseau :
1. Le pair obtient une référence distante RMI sur l'annuaire P2P via le RMI Registry
2. Le pair se connecte sur l'annuaire et obtient une connexion.
3. Le pair utilise cette connexion pour obtenir la liste des pairs déjà enregistrés sur l'annuaire (étape 0) qui possèdent le fichier
voulu.
4. Le pair d'adresse ensuite directement au(x) pair(s) qui possède(nt) ce fichier dans son intégralité (ou des morceaux

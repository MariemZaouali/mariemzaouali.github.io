...
# ==============================
# 4. Download Custom TEST Dataset from Google Drive
# ==============================
FILE_ID = "1fnKF7UWP5PMzoL888UptJoK92dRhvYy6"   # <-- mettre l'ID du fichier ici
DEST_PATH = "cifar10_test.pt"

def download_file_from_google_drive(file_id, destination):
    """
    Download large files from Google Drive using a confirmation token.
    """
...
# Load the custom CIFAR-10 test file
test_data = torch.load(DEST_PATH)
test_images = test_data["images"]     # shape: [N, 3, 32, 32]
test_labels = test_data["labels"]     # dummy or -1 labels

# Wrap as TensorDataset (transforms already applied if needed)
test_dataset = TensorDataset(test_images, test_labels)

print("Custom test dataset loaded:", test_images.shape)

# ==============================
# 5. Train Dataset (unchanged)
# ==============================
...
# ==============================
# 8. Training Loop + Save Prediction File
# ==============================
...
for name, model in models_dict.items():
    ...
    _, _, test_preds = evaluate(model, test_loader)
    submission = pd.DataFrame({
        "Id": list(range(len(test_preds))),  # keeps the ID
        "Predicted": test_preds
    })
    submission_file = f"{name}_CIFAR10_test_predictions.csv"
    submission.to_csv(submission_file, index=False)
    print(f"\nTest predictions saved to {submission_file}")

...
```link
https://drive.google.com/file/d/1fINk_zhi_Yjo3LSf1eFMnXeZnLgTQXFs/view?usp=drive_link
```
```cmd
ssh-keygen -f "/home/bigdata/.ssh/known_hosts" -R "localhost"
ssh localhost
...
1. Pour ce faire, nous allons créer une base de données **postgres**. Installer postgres avec la commande:
```cmd
...
```
Vous venez de créer votre table sous la base de données **postgres**. Tapez la commande suivante pour visualiser les tables
```cmd
\dt
```
...
```cmd
mv Downloads/order_detail.csv order_detail.csv
```
Connectez-vous postgres et lancez
```cmd
COPY order_detail FROM '/home/bigdata/order_detail.csv' DELIMITER ',' CSV HEADER;
```
Vérifiez que les données ont été chargées avec succès:
```cmd
SELECT * FROM order_detail LIMIT 5;
```
Tapez pour quitter postgres:
```cmd
\q
```
3.Connectez-vous à hive.
```cmd
hive
```
...
quit;
```
6. Ensuite, utiliser **sqoop** sur votre machine qui permmettra de charger les données de **postgres** vers **hive**. Lancez la commande de sqoop
 ```cmd
```
![image](https://github.com/user-attachments/assets/978f1309-c4b9-46f0-8eb4-feb6b417d6b7)

7.	Accédez à « localhost :8088 » pour voir le succès de votre transfert de données avec sqoop. La raison pour laquelle vous trouvez ce transfert sur le history server de hadoop est le fait que Hive a déclenché un job MapReduce pour faire cette opération !

8.	Vérifier le succès du transfert en revenant vers hive:
```cmd

...
1. Pour ce faire, nous allons créer une base de données **postgres**. La table à créer s'appelle **order_detail**
...
```
que vous allez charger à partir du fichier csv : **order_detail.csv** à partir du lien :
```link
https://drive.google.com/drive/folders/1yHi5gPFu_4999GhdemwRD8ux2mllufU8?usp=sharing
```

3. Ensuite, installer **sqoop** sur votre machine qui permmettra de charger les données de **mysql** vers **hive**.

4.	Créer avec **hive cli** la base de données **lab2_big_data**.

5. Lancez la commande de sqoop
 ```cmd
```
![image](https://github.com/user-attachments/assets/978f1309-c4b9-46f0-8eb4-feb6b417d6b7)

6.	Accédez à « localhost :8088 » pour voir le succès de votre transfert de données avec sqoop. La raison pour laquelle vous trouvez ce transfert sur le history server de hadoop est le fait que Hive a déclenché un job MapReduce pour faire cette opération !
7.	Vérifier le succès du transfert en revenant vers hive:
```cmd
